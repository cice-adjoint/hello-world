...
!          Cecilia M. Bitz, UW
      ...
      subroutine runtime_diags (dt, istep1 ,  ap_istep1 , apn_istep1, hpn_istep1)
...
!      use icepack_meltpond_cesm, only : apnd,hpnd  ! luyang 20190630
      use icedrv_state, only: trcrn   !luyang20190702
      ...
      integer (kind=int_kind) :: &   !luyang20190702  !luyang 20190705
         ntrcr, nt_apnd = 19, nt_hpnd = 20
...
!        ap_istep1 = sum( trcrn ( 3, nt_apnd,:) )
!        hp_istep1 = sum( trcrn ( 3, nt_hpnd,:) )/5.0_dbl_kind
        ap_istep1 = trcr (3,nt_apnd)
        hp_istep1 =  trcr (3,nt_hpnd)
        apn_istep1 = trcrn ( 3, nt_apnd,:)
        hpn_istep1 = trcrn ( 3, nt_hpnd,:)

!        write(*,*) 'n=', n
!        write(*,*) 'nx_names=', nx_names(n)
!        write(*,*),'ap_istep1_indiag=', ap_istep1
!        write(*,*) 'pond area fraction      =',sum( trcrn (n,nt_apnd,:) )
!        write(*,*) 'apn_istep1      =', trcrn ( 3, nt_apnd,:)
!        write(*,*) 'hpn_istep1      =', trcrn ( 3, nt_hpnd,:)
        !-----------------------------------------------------------------
        ! start spewing
        !-----------------------------------------------------------------
        ...
        write(nu_diag_out+n-1,*) '----------pnd----------'
!        write(nu_diag_out+n-1,900) 'pond area fraction     =',apnd! pond area
!        write(nu_diag_out+n-1,900) 'pond depth             =',hpnd! pond depth
!        write(nu_diag_out+n-1,900) 'pond area fraction     =',sum( trcrn (n,nt_apnd,:) )! pond area
!        write(nu_diag_out+n-1,900) 'pond depth             =',sum( trcrn (n,nt_hpnd,:) )/5.0_dbl_kind! pond depth
        write(nu_diag_out+n-1,900) 'pond area fraction      =',trcr (n,nt_apnd)! pond area
        write(nu_diag_out+n-1,900) 'pond depth              =',trcr (n,nt_hpnd)! pond depth

        write(nu_diag_out+n-1,900) 'ap_istep1               =',ap_istep1
      end do
899   format (43x,a24)
