...
! authors: William H. Lipscomb, LANL
      ...
      subroutine step_therm1 (dt    , pndaspect  , istep1)                              !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! luyang!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real (kind=dbl_kind)  :: &
         pndaspect  ! ratio of pond depth to area fraction                                         !!!!!!!!!!!!!!!!!!! luyang!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!write(*,*) 'nx=',nx
!do kk=1,nt_qsno
!write(*,*) 'trcrn(i,nt_qsno:nt_qsno+nslyr-1,:)=',&
!(trcrn(i,kk,1),kk=1,nt_qsno)
!write(*,*),trcrn(i,kk,1)
!enddo
!write(*,*) 'nt_apnd=',nt_apnd
!write(*,*) 'nt_hpnd=',nt_hpnd
!write(*,*) 'trcrn       (i,nt_apnd,:)=', trcrn       (i,nt_apnd,:)
!110 format(1X,'F16.6/')


!write(*,*) " before calling icepack_step_therm1"
!write(*,*) " nx =" ,nx
!write(*,*) "aicen       (i,:)=" , aicen       (i,:)
...
!call luyangoutput( aicen (i,:), vicen(i,:), &
!                   trcrn (i,nt_Tsfc,:)    , &
!                   meltsn(i,:),  melttn(i,:), &
!                   frain(i) , istep1 )


!write(*,*) " after calling icepack_step_therm1"
!write(*,*) " nx =" ,nx
!write(*,*) "aicen       (i,:)=" , aicen       (i,:)


            call luyangoutput_in_step( aicen (3,:),  vicen(3,:) , &
                               trcrn (3,nt_Tsfc,:)      , &
                               meltsn(3,:),  melttn(3,:), &
                               frain(3) , istep1 )
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      subroutine luyangoutput( aicen, vicen, Tsfc,  meltsn,  melttn, frain ,istep1)
!      use icedrv_calendar, only: istep1
      ...
      integer (kind=int_kind) :: &
         istep1     ! counter, number of steps at current timestep
            ...
            open(12,file="./"//'melttn_'//char_count//".txt") !
            write(12,200)melttn
            close(12)

            open(12,file="./"//'meltsn'//char_count//".txt") !
            write(12,200)meltsn
            close(12)

            open(12,file="./"//'frain'//char_count//".txt") !
            write(12,200)frain
            close(12)

            open(12,file="./"//'aicen'//char_count//".txt") !
            write(12,200)aicen
            close(12)

            open(12,file="./"//'vicen'//char_count//".txt") !
            write(12,200)vicen
            close(12)

            open(12,file="./"//'Tsfc'//char_count//".txt") !
            write(12,200)Tsfc
            close(12)
      ...
      end subroutine luyangoutput



      subroutine luyangoutput_in_step( aicen, vicen, Tsfc,  meltsn,  melttn, frain ,istep1)
     ...
     real (kind=dbl_kind), dimension(:)  :: &
      ...
      integer (kind=int_kind) :: &
         istep1     ! counter, number of steps at current timestep
            ...
            data_dir='/home/luy/icepack/intermediate_forcing_data/dir3/'


            open(12,file=trim(adjustl(data_dir))//'melttn'//char_count//".txt") !
            write(12,200)melttn
            close(12)

            open(12,file=trim(adjustl(data_dir))//'meltsn'//char_count//".txt") !
            write(12,200)meltsn
            close(12)

            open(12,file=trim(adjustl(data_dir))//'frain'//char_count//".txt") !
            write(12,200)frain
            close(12)

            open(12,file=trim(adjustl(data_dir))//'aicen'//char_count//".txt") !
            write(12,200)aicen
            close(12)

            open(12,file=trim(adjustl(data_dir))//'vicen'//char_count//".txt") !
            write(12,200)vicen
            close(12)

            open(12,file=trim(adjustl(data_dir))//'Tsfc'//char_count//".txt") !
            write(12,200)Tsfc
            close(12)
      ...
      end subroutine luyangoutput_in_step
