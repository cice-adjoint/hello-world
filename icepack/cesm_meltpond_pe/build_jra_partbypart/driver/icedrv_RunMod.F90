...
!  author Elizabeth C. Hunke, LANL
      ...
      subroutine icedrv_run ( pndaspect , apond_timeseries, hpond_timeseries , apond_timeseriesn, hpond_timeseriesn)                        !-------------------------------------------- luyang --------------------------------------------
...
!                 pndaspect = 0.7_dbl_kind

                 call ice_step ( pndaspect ,apond_istep, hpond_istep ,istep1, apn_istep1, hpn_istep1)

                 apond_timeseries (istep1) = apond_istep
                 hpond_timeseries (istep1) = hpond_istep

                 do nncat = 1,5
                    apond_timeseriesn (istep1, nncat) = apn_istep1(nncat)
                    hpond_timeseriesn (istep1, nncat) = hpn_istep1(nncat)
                 enddo
...
!  author Elizabeth C. Hunke, LANL
      ...
      subroutine ice_step ( pndaspect ,apond_istep, hpond_istep ,istep1 , apn_istep1,hpn_istep1)
...
!     use icedrv_diagnostics, only: icedrv_diagnostics_debug
      ...
      integer (kind=int_kind) :: &   !luyang20190702
...
!      call icedrv_diagnostics_debug ('post prep_radiation')
      ...
      call step_therm1     (dt      ,pndaspect   , istep1) ! vertical thermodynamics
...
!      call icedrv_diagnostics_debug ('post step_rad, cpl')
      ...
      if (mod(istep,diagfreq) == 0) then
         call runtime_diags(dt, istep1 ,  apond_istep ,apn_istep1, hpn_istep1)       ! log file

!         write(*,*), 'apond_istep=', apond_istep
...
!      apond_istep  = sum(trcrn (1,nt_apnd,:) ) ! pond area
!      hpond_istep  = sum(trcrn (1,nt_hpnd,:) ) /5.0_dbl_kind ! pond depth
